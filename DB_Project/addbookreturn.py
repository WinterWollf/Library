...
def book_return(list_user, list_book, return_date_calendar, librarian_id, window):
    if list_user == ["", "", "", ""]:
        tkinter.messagebox.showinfo("Błąd", "Błędne dane! Nie wprowadzono czytelnika!")
    elif list_book == ["", "", "", ""]:
        tkinter.messagebox.showinfo("Błąd", "Błędne dane! Nie wybrano książki!")
    else:
        ...
        if return_date < formatted_date_return:
            tkinter.messagebox.showinfo("Błąd", "Błędne dane! Data zwrotu nie może być mniejsza niż data wypożyczenia!")
        else:
            ...
            tkinter.messagebox.showinfo("Komunikat", "Zwrot dokonany pomyślnie!")

            if belated == 1:
                tkinter.messagebox.showinfo("Komunikat", "Zwrot dokonany z opóźnieniem!")

            # Oblicz różnicę w dniach między potential_return a return_date
...
def book_search(window, text, mycursor, librarian_id, list_book, list_user):
    ...
    if search_text == "":
        tkinter.messagebox.showinfo("Błąd", "Nie znaleziono czytelnika o podanym imieniu i nazwisku!")
    else:
        # Zapytanie SQL, które zwraca wszystkie książki wypożyczone przez danego użytkownika na podstawie jego user_id
