...
ASSETS_PATH = OUTPUT_PATH / Path(r"C:\Users\wikto\OneDrive\Pulpit\Bazy-Danych\DB Project\Assets\LibrarianPage")
...
def log_out(window):
    ...
    tkinter.messagebox.showinfo("Komunikat", "Nastąpiło poprawne wylogowanie z systemu!")


def make_backup(security_level):
    if security_level == 3:
        text = backup.make_backup()
        tkinter.messagebox.showinfo("Komunikat", f"Pomyślnie utworzono kopię zapasową bazy danych do pliku {text}")
    else:
        tkinter.messagebox.showinfo("Odmowa", f"Brak uprawnień do tworzenia kopii zapasowej")
...
def add_reader(window, librarian_id, security_level):
    if security_level >= 2:
        ...
        new_window = Tk()
        new_window.title("Dodawanie nowego czytelnika - eAGH")
        new_window.geometry("1000x800")

        addreader.show_addreader_page(new_window, librarian_id)
    else:
        tkinter.messagebox.showinfo("Odmowa", f"Brak uprawnień do dodawania nowych czytelników")
...
def add_book(window, librarian_id, security_level):
    if security_level >= 2:
        ...
        new_window = Tk()
        new_window.title("Dodawanie nowej pozycji - eAGH")
        new_window.geometry("1000x800")
        ...
        tkinter.messagebox.showinfo("Odmowa", f"Brak uprawnień do dodawania nowych książek")
...
def add_return(window, librarian_id):
    ...
    new_window = Tk()
    new_window.title("Dodawanie nowego zwrotu - eAGH")
    new_window.geometry("1000x800")
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    tkinter.messagebox.showinfo("Błąd", "Nastąpił błąd podczas generowania pliku!")
    else:
        tkinter.messagebox.showinfo("Odmowa", f"Brak uprawnień potrzebnych do generowania kodów QR")
...
def raport_page(window, librarian_id, security_level):
    if security_level == 3:
        ...
        window = Tk()
        window.title("Generator raportów - eAGH")
        window.geometry("1000x800")

        raportapage.show_raport_page(window, librarian_id)
    else:
        tkinter.messagebox.showinfo("Odmowa", f"Brak uprawnień potrzebnych do generowania raportów")
