...
ASSETS_PATH = OUTPUT_PATH / Path(r"C:\Users\wikto\OneDrive\Pulpit\Bazy-Danych\DB Project\Assets\AddReaderPage")
...
def random_data_generator(db, mycursor, name, surname, reader_id):
    ...
    while status != True:
        # Sprawdzenie, czy username występuje w tabeli Users
    ...
    # Zapytanie SQL do dodania nowego czytelnika do tabeli Readers
...
def add_user(n, s, a, t, e, window, librarian_id):
    ...
    # Sprawdzenie, czy mail występuje w tabeli Readers
    ...
    if len(name) == 0:
        tkinter.messagebox.showinfo("Błąd danych", "Błędne imię!")
    elif len(surname) == 0:
        tkinter.messagebox.showinfo("Błąd danych", "Błędne nazwisko!")
    elif len(telephone_number) == 0 or telephone_number.isdigit() == False:
        tkinter.messagebox.showinfo("Błąd danych", "Błędny numer telefonu!")
    elif main_occur is True:
        tkinter.messagebox.showinfo("Błąd danych", "Podany mail należy już do innego czytelnika!")
    else:
        # Zapytanie SQL do dodania nowego czytelnika do tabeli Readers
        ...
        tkinter.messagebox.showinfo("Komunikat", "Czytelnik został dodany")
        tkinter.messagebox.showinfo("Dane logowania", f"Nazwa użytkownika: {username}\nHasło: {password}")
