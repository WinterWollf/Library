...
ASSETS_PATH = OUTPUT_PATH / "Assets" / "UserOverView"
...
def back_to_serach_page(window, result, librarian_id, status):
    window.destroy()
    ...
    usersearch3.show_search_page(new_window, librarian_id, result, status)


def show_user_overview(window, user_id, result, librarian_id, status):
    align_window(window)
    ...
    button_2 = Button(
        ...
        borderwidth=0,
        highlightthickness=0,
        command=lambda: back_to_serach_page(window, result, librarian_id, status),
        relief="flat"
    )
    button_2.place(
        x=667.0,
