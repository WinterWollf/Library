...
def back_add_book_return(window, librarian_id, list_book, list_user):
    window.destroy()
...
def show_search_page(window, result, librarian_id, list_book, list_user):
    ...
    def on_select(event):
        ...
        new_window = Tk()
        new_window.title("Book Review")
        new_window.geometry("1000x800")

        bookoverview_3.show_book_overview(new_window, result[index[0]][2], result, librarian_id, list_book, list_user)
    ...
    button_2 = Button(
        ...
        borderwidth=0,
        highlightthickness=0,
        command=lambda: back_add_book_return(window, librarian_id, list_book, list_user),
        relief="flat"
    )
    button_2.place(
        x=400.0,
