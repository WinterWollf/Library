...
def send_email(status, user_data, book_data, return_date=None, day_after=0):
    ...
    if status == 0:
        if day_after <= 0:
            subject = "Confirmation of return"
            message = f"Hi {user_data[0]} {user_data[1]}. We confirm the successful return of the book entitled {book_data[0]}, {book_data[1]}. See you soon!"
        else:
            subject = "Confirmation of return"
            message = f"Hi {user_data[0]} {user_data[1]}. We confirm the successful return of the book entitled {book_data[0]}, {book_data[1]}. The book was returned {day_after} days after the deadline! Please report to the librarian on your next visit."
    else:
        subject = "Confirmation of rental"
        message = f"Hi {user_data[0]} {user_data[1]}. We confirm the successful loan of the book entitled {book_data[0]}, {book_data[1]}. The book should be returned to {return_date}. Enjoy reading!"
    ...
    server = smtplib.SMTP('smtp.gmail.com', 587) # For Gmail usage
    server.starttls()
