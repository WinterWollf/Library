...
ASSETS_PATH = OUTPUT_PATH / Path(r"C:\Users\wikto\OneDrive\Pulpit\Bazy-Danych\DB Project\Assets\AddBookPage")
...
def add_book(t, a, r, p, g, pr, window, librarian_id, book_id):
    ...
    if len(title) == 0:
        tkinter.messagebox.showinfo("Błąd danych", "Błędny tytuł!")
    elif len(author) == 0:
        tkinter.messagebox.showinfo("Błąd danych", "Błędny autor!")
    elif release_date > current_date:
        tkinter.messagebox.showinfo("Błąd danych", "Błędna data wydania!")
    elif not is_valid_price(price):
        tkinter.messagebox.showinfo("Błąd danych", "Błędna cena!")
    else:
        book_genre_id = get_genre_id(mycursor, book_genre_name)
        if book_genre_id is None:
            tkinter.messagebox.showinfo("Błąd danych", "Błędny gatunek!")
            return
        ...
        if book_id == 0:
            ...
            tkinter.messagebox.showinfo("Komunikat", "Książka została dodana pomyślnie")
            ...
            tkinter.messagebox.showinfo("Komunikat", "Dane książki zostały pomyślnie zaktualizowane!")
            back_to_librarianpage(window, librarian_id)
...
def show_add_book_page(window, librarian_id, book_data):
    ...
    # Pobierz nazwy gatunków z bazy danych
    genres = get_genre_names(mycursor)

    # Utwórz StringVar do przechowywania wybranego gatunku
    ...
    # else:
    selected_genre.set(genres[0])  # Ustaw domyślnie pierwszy gatunek

    # Utwórz ttk.Combobox i umieść go na canvas
    ...
    if book_data[4] == "":
        default_date = "2024-06-15"  # Przykładowa data
    else:
        default_date = str(book_data[4])

    # Konwertuj string na datę
    default_date_obj = datetime.datetime.strptime(default_date, "%Y-%m-%d")

    # Utwórz DateEntry z domyślną datą
